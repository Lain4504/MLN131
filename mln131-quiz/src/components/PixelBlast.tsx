...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (!container) {
      console.warn('PixelBlast: Container not found');
      return;
    }
    speedRef.current = speed;
    const needsReinitKeys = ['antialias', 'liquid', 'noiseAmount'];
    const cfg: { antialias: boolean; liquid: boolean; noiseAmount: number } = { antialias, liquid, noiseAmount };
    ...
    if (mustReinit) {
      ...
      if (!renderer.getContext()) {
        console.error('PixelBlast: WebGL context not available');
        return;
      }
      
      // Ensure container has size
      const containerWidth = container.clientWidth || window.innerWidth || 1920;
      const containerHeight = container.clientHeight || window.innerHeight || 1080;
      ...
      console.log('PixelBlast: Renderer initialized', {
      ...
      try {
        ...
        console.log('PixelBlast: ShaderMaterial created successfully');
      } catch (error) {
        console.error('PixelBlast: Failed to create ShaderMaterial', error);
        return;
      }
      ...
      const setSize = () => {
        ...
        if (w === 0 || h === 0) {
          console.warn('PixelBlast: Container has zero size', { w, h });
          return;
        }
        ...
        console.log('PixelBlast: Size set', {
          container: { w, h },
      ...
      // Force resize after a short delay to ensure container has size
      setTimeout(() => {
        setSize();
      }, 100);
      ...
      const animate = () => {
        ...
        const canvasComputed = window.getComputedStyle(renderer.domElement);
        console.log('PixelBlast: First frame rendered', {
        ...
        // Log error if canvas is not visible after 60 frames
        if (frameCount === 60) {
          const canvasVisible = renderer.domElement.offsetWidth > 0 && renderer.domElement.offsetHeight > 0;
          if (!canvasVisible) {
            console.error('PixelBlast: Canvas not visible after 60 frames', {
              canvasSize: { w: renderer.domElement.width, h: renderer.domElement.height },
              canvasOffset: { w: renderer.domElement.offsetWidth, h: renderer.domElement.offsetHeight },
              containerSize: { w: container.clientWidth, h: container.clientHeight }
            });
          }
        }
